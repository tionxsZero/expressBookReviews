...
public_users.post("/register", (req, res) => {
    ...
    if (!username || !password) res.status(403).send({ message: "Username or password must be valid" });

    const isvalid = isValid(username);

    if (isvalid) return res.status(403).json({ message: "user already registered" })
    else {
...
public_users.get('/review/:isbn', function (req, res) {
    ...
    return new Promise((resolve, reject) => {
        ...
        res.status(200).json(bookFiltered)
    ).catch((error) => {
        res.status(404).json({ error: error.message })
    }
    )
